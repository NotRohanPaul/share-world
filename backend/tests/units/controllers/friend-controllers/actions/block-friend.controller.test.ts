import { blockFriendController } from "@src/controllers/friend-controllers/actions";
...
describe("test for unblock user controller", () => {
    ...
    it("respose 200 if receiver is a friend of sender before and now its blocked", async () => {
        ...
        await blockFriendController(mockRequest, mockResponse, mockNext);

        expect(mockResponse.sendStatus).toHaveBeenCalledWith(200);
    });

    it("respose 400 if receiver is not already friend of sender", async () => {
        const { user1, user2 } = await createUsers();
