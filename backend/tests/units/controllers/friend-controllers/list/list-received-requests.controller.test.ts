import { listReceivedRequestsController } from "@src/controllers/friend-controllers/list";
...
describe("test for list received request", () => {
    ...
    it("reponse 200 with all received request list", async () => {
        ...
        const users = await UsersModel.insertMany(usersList);
        ...
        const emailsListExceptOne = users.filter(({ email }) => email !== testUserEmail).map(({ _id }) => {
            return {
                sender: _id.toHexString(),
                receiver: testUserId.toHexString()
            };
        });
        await FriendRequestsModel.insertMany(emailsListExceptOne);
        ...
        await listReceivedRequestsController(mockRequest, mockResponse, mockNext);
